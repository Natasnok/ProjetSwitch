...
def Calcul_mental():
    global question,answer,score_calcul
    ...
    while not game_over:
        ...
        if time_left <= 0:
            ...
            draw_text(f"Score: {score_calcul}", FONT, BLACK, screen, 10, 10)
            draw_text(f"Temps restant: {time_left} s", FONT, BLACK, screen, WIDTH - 310, 10)
        ...
        # Limiter la vitesse de la boucle de jeu
        clock.tick(30)
...
def Jeu_Des_Fleches():
    global player_x, player_y, score, arrow_delay, ARROW_WAVE_DELAY, game_over, arrow_count, arrows, CIRCLE_CENTER, CIRCLE_RADIUS, PLAYER_RADIUS, ARROW_SIZE
...
def Memory():
    global selected_tiles,matched_tiles,numbers
...
def Snake():
    global Length_of_snake, x1, y1, x1_change, y1_change, snake_List, Length_of_snake, score_snake, foodx, foody
    start_time = time.time()
    game_duration = switch_time
    snake_block = 10
    snake_speed = 15
    ...
    while not game_over:
        ...
        while not game_close:
            ...
            score_text = font.render("Score: " + str(score_snake), True, (255, 255, 255))
            screen.blit(score_text, (10, 10))  # Afficher le score dans le coin supérieur gauche
        ...
        while game_close:
            ...
            Length_of_snake = 1
            score_snake = 0
            foodx = round(random.randrange(60, WIDTH - 60) / 10.0) * 10.0
            foody = round(random.randrange(60, HEIGHT - 60) / 10.0) * 10.0
            return

# Liste des jeux
jeux = [Jeu_Des_Fleches, Calcul_mental, Memory, Snake]

global switch_time, nb_switch

current_game = None
next_game=True
new_game=None
start_time = 0
switch_time = 3  # Time in seconds before switching to a new game
arrows = []
player_x = WIDTH // 2
player_y = HEIGHT // 2
arrow_count = 0
